...
```{r}
names(allWRIGHTtext) = c("title","text")
input=allWRIGHTtext 

n.topics=32

mallet.instances <- mallet.import(input$title, input$text, stoplist.file="data/stop_words.txt",
...
rownames(doc.topics) = input$title
...
allcounts <- as_data_frame(doc.topics) %>%
  mutate(document = rownames(allcounts))
topicCounts = allcounts %>% 
  gather(topic,proportion,-document)
