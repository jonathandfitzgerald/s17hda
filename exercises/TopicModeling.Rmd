...
wordFrame = topic.words %>% as.data.frame()
...
Then, let's try to get a sense of what words are shared across topics. 
...
gatherWords %>% filter(word == word2search) %>% 
  ...
  ggtitle(paste("Number of times the word", word2search, "appears")) +
...
Or perhaps we may want to look at particular topics to see their most relevant texts (you'll probably need to change `topic2search` to a topic that appears in your date):
```{r}

topic2search = "skinner boston peace mose jubilee"
