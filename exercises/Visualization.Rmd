...
If we simply want a count of all observations, we can make the x-axis contains all the information by just setting it to an empty string and filling the bars based on a variable (in this case, the cluster id). The bars will appear stacked on top of each other.
...
If you dig into the `theme()` arguments (the easiest way to do this is to type `theme(` and then hit `tab` to scroll through the available options), you'll see that you can customize almost anything about your chart. Just for fun:
