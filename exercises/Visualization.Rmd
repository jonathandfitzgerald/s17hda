...
If we simply want a count of all observations, we can make the x-axis contain all the information by just setting it to an empty string and filling the bars based on a variable (in this case, the cluster id). The bars will appear stacked on top of each other.
...
1. Read in the `crewlists.csv` file we used a few weeks back. What would be a good variable to create a histogram from? What about a bargraph? Create one of each for different variables. Experiment with the settings of each until you have visualizations you'd be happy to share in class next week (and be ready to talk about what you think each illustrates). 
...
vtClusters %>% group_by(corpus) %>% count() %>% ggplot() + geom_bar(stat = "identity", aes(x=corpus, y=n, fill=corpus))
...
vtClusters %>% 
  ...
  slice(1) %>%
  ggplot() +
  geom_bar(stat = "identity", aes(x=reorder(as.character(cluster), -count), y=count, fill=as.character(cluster))) 
...
vtClusters %>% 
  ...
  ungroup %>%
  ggplot() +
  geom_bar(stat = "identity", aes(x=reorder(X1, -count), y=count, fill=as.character(cluster))) +
