...
`ggplot` also lets use overlay different kinds of plots. Can you tell what's happening below?
...
Remember that we can facet our graphs by particular variables. What's happening below?

```{r}
ggplot(census %>% filter(state %in% ne)) + 
  geom_point(mapping = aes(x = Newspapers, y = PrintingOffices)) +
  facet_grid(. ~ state)
```
